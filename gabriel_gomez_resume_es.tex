...
{Desarrollé un sistema POS de escritorio con Electron y NestJS para un cliente de retail, con almacenamiento SQLite offline-first, impresión térmica y de etiquetas por USB, soporte para lectores de código de barras y gestión de inventario con trazabilidad completa mediante auditorías para reducir discrepancias de stock.}
\resumeItem{Afinia PQR}
{Adapté el sistema white-label de gestión de cuadrillas para el proceso de PQR (peticiones, quejas y reclamos) de la empresa eléctrica Afinia, como contratista freelance para Extreme Technologies. Construí la app Android y el web service Jakarta soportando un flujo de trabajo de tres roles: un administrador crea el caso y lo asigna a una cuadrilla contratista, el contratista envía un reporte de campo marcando el trabajo como resuelto o con seguimiento pendiente, y un supervisor revisa el resultado antes de cerrar el caso. Implementé la máquina de estados que maneja las transiciones del caso y el enrutamiento de formularios por rol entre la app móvil y el backend.}
\resumeItemListEnd
